...
- Host context injection is scoped: only memory entries whose effective `injection_policy` is `global_context` enter the shared bounded summary. Project, repo, domain, host-native, local-only, and low-priority memories remain available to the local registry/index and future project-aware context compilation, but do not become global host context by default.
