...
- Host context injection is scoped: only memory entries whose effective `injection_policy` is `global_context` and pass the global-candidate gate enter the shared bounded summary. Legacy/nightly synthesis rows are not promoted into global host context without canonical, manual, or approved markers. Project, repo, domain, host-native, local-only, and low-priority memories remain available to the local registry/index and future project-aware context compilation, but do not become global host context by default.
