...
`OpenRelix™` 意为开源的个人记忆珍藏，是一套面向 AI coding agents / AI CLI 的本地优先资产层，当前公开口径是 `v0.1.0 预览版`。它不绑定某一个 AI host；当前首个公开适配器是 Codex CLI / Codex app-server，因为 Codex 已经提供可落地的 thread、history、session、skill 和 memory 表面。
...
| 项目 / host | 预期状态 | 说明 |
| --- | --- | --- |
| Codex CLI | 当前支持 | `v0.1.0 预览版` 的稳定回退采集面。 |
