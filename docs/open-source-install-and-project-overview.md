# OpenKeepsake™ 开源安装说明与项目说明

## 一句话说明

`OpenKeepsake™` 是一套面向 AI coding agents / AI CLI 的本地优先资产层，当前公开口径是 `v0.1.0 预览版`。它不绑定某一个 AI host；当前首个公开适配器是 Codex CLI，因为 Codex 已经提供可落地的 history、session、skill 和 memory 表面。

GitHub 项目页：[Ray1Ren/openkeepsake](https://github.com/Ray1Ren/openkeepsake)。如果这个项目对你有帮助，欢迎点星支持。
...
- `zsh`
- Python 3.10+
- 当前 v0.1.0 预览版适配器所需的 Codex CLI，以及可写的 `CODEX_HOME`，默认是 `~/.codex`

Linux / Windows 暂时作为后续目标，不在 `v0.1.0 预览版` 对外承诺里。底层 Python 脚本仍尽量保留可配置路径，但一键安装、后台刷新、夜间整理和防睡眠策略都先按 macOS 收口。

当前 `v0.1.0 预览版` 的公开安装器先支持 Codex CLI 适配器。很多已落地能力依赖 Codex 专属表面，包括 `CODEX_HOME`、Codex history / session 文件、Codex native memory、Codex skills 和 Codex custom prompts。产品定位仍是 AI-agent-first；其他 CLI host 后续可以通过 adapter 层接入同一套本地资产模型，但不属于当前公开安装承诺。

---

## v0.1.0 预览版发布说明

`v0.1.0 预览版` 是 OpenKeepsake 的首个公开预览版本，目标是先把“本地个人 AI 资产系统”的核心闭环跑通：安装、采集、整理、复盘、记忆登记、面板可视化和本地自动化。

当前版本已经对外承诺的范围：

- **平台**：macOS。
- **安装方式**：`npx openkeepsake install` 或源码目录下 `./install/install.sh`。
- **当前适配项目**：Codex CLI。默认读取 Codex CLI 的 `history.jsonl` 和 `sessions/**/*.jsonl`，并把 repo 内维护的 skill / prompt / shell 入口安装到 Codex 用户环境。
- **预览能力**：Codex app-server 线程读取是 opt-in 能力，安装时加 `--read-codex-app` 后才会尝试；失败时会回退到稳定的 Codex CLI 文件采集。

预期后续支持的项目 / host：

| 项目 / host | 预期状态 | 说明 |
| --- | --- | --- |
| Codex CLI | 当前支持 | `v0.1.0 预览版` 的首个公开适配器。 |
| Codex app-server / Codex 应用线程 | 预览读取 | 需要显式开启 `--read-codex-app`，当前不作为默认来源。 |
...
- GitHub README 首屏截图，包含 `OpenKeepsake™`、安装命令和仓库 URL。
- npm package 页面截图，包含 `openkeepsake` 包名、版本号和 `npx openkeepsake install`。
- GitHub Release `v0.1.0` 页面截图。
- GitHub Pages 展示页截图。
- 对应的 git tag、npm publish 时间和 release 发布时间。

商标标识使用 `OpenKeepsake™` / `okeep™`；不要使用 `®`，除非相关商标已经在对应司法辖区完成注册。
...
兼容旧安装：如果未显式设置 state root，且 legacy state root 存在而新的 `openkeepsake` state root 不存在，运行时会先复用旧目录，避免改名后看不到历史数据。
...
默认安装只读取稳定的 Codex CLI `history.jsonl` 和 `sessions/**/*.jsonl`。如果你明确要读取 Codex 应用线程，安装时额外加 opt-in 参数；它会优先尝试 `codex app-server`，不可用时回退到 CLI history/session 文件：
...
3. 在当前 Codex 适配器下按默认 `codex-context` 开启 memories/history，并同步一份 bounded summary
...
个人记忆系统默认开启，并且默认模式是 `codex-context`：一份完整的本地资产记忆记录到本项目的 state root，同时把压缩后的 bounded summary 同步进当前 host native context。用户可以按需要显式选择只本地记录，或关闭本系统本地记忆写入。

bounded summary 的压缩策略保持轻量：同签名记忆跨天归并，durable / session 优先进入上下文，low-priority 默认只留本地；默认 token budget 是 target 4.2K、warn 4.6K、max 5K，避免把原始窗口或完整 registry 塞进 host context。
...
4. 安装全局 `okeep` 命令，并在需要时把对应用户 bin 目录写入 shell `PATH`
5. 安装 macOS 后台 refresh 服务
...
- `--memory-mode local-only|codex-context|off`
  指定记忆模式，默认 `codex-context`
- `--record-memory-only`
  记录个人记忆，但不往 host native context 注入记忆摘要；等价于 `--memory-mode local-only`
- `--use-codex-context`
  使用当前 Codex 适配器的 host native memory context；等价于 `--memory-mode codex-context`
- `--disable-personal-memory`
...
如果你使用的 AI host 支持仓库内 skill 发现，仓库下 `.agents/skills/` 里的 skills 会被自动发现。当前 v0.1.0 预览版适配器使用 Codex 的 repo-local skill 发现机制。

这适合“只在当前仓里用”的能力。

### 2. 任意仓库下启动

如果你希望这些 skills 在任何仓库里启动 AI host 都能用，就需要把它们安装到该 host 的用户级 skill 目录。当前 v0.1.0 预览版会安装到 Codex 用户级 skill 目录。
...
安装完成后，主推荐入口是在新的 AI agent 线程里直接输入；当前 v0.1.0 预览版适配器对应 Codex 线程：
...
7 天学习命令是可选动作，只在需要补齐 OpenKeepsake 本地记忆和日报时运行：

```bash
okeep review --date "$(date +%F)" --learn-window-days 7
```

这一步不会在安装时自动执行，因为它可能需要一点时间。它会先补齐最近 7 天缺失或还不是 final 的日报，且自动补齐阶段不会给每个历史日期再扩展一层学习窗口；随后再读取最近 Codex 窗口，更新本系统本地 memory 和 overview。默认 `codex-context` 会同步 bounded summary 到 Codex native context，但不会写入原始窗口或完整 registry；如果使用 `--record-memory-only`，则只更新本地 state root。如果没有安装全局 `okeep` 命令，installer 会打印一条带 `AI_ASSET_STATE_DIR` / `CODEX_HOME` 的 `python3 scripts/okeep.py ...` fallback 命令。
...
- `okeep mode`
  查看当前记忆模式；安装后要切换模式时使用 `okeep mode codex-context`、`okeep mode local-only` 或 `okeep mode off`，不需要重新安装
- `okeep review`
...
当前 plugin draft 只是草案包装层，不作为 `v0.1.0 预览版` 的公开安装入口。

首发推荐只讲 installer-first：

- 通过 `./install/install.sh` 初始化 state root
- 通过 symlink 把 repo skill 挂到用户级 `~/.codex/skills/`
- 通过 `okeep` 和 macOS LaunchAgent 接上本地自动化

插件目录保留在仓里，是为了后续把同一套 canonical skills 打包成 Codex plugin。等 public metadata、截图、policy URL 和发布流程收口后，再把 marketplace 状态切回可安装。
...
如果还要读取 Codex 应用线程，需要显式加：

```bash
./install/install.sh --profile integrated --read-codex-app
```
...
- 完整安装：再把 skill、memory、`okeep` 命令、后台刷新、夜间整理和任务期间防睡眠一起接上
